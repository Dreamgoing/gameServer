...
func main() {
	...
	session, err := mgo.Dial("localhost")  //连接数据库
	if err != nil {
		panic(err)
	}
	defer session.Close()
	...
	err = collection.Insert(&Child{"wang","+1234 ","12"})
	if err != nil {
		panic(err)
	}
	...
	rechild := Child{}
	err = collection.Find(bson.M{"name":"wang"}).One(&rechild)
	fmt.Println(rechild)
	...
	_, err = collection.RemoveAll(bson.M{"name":"wang"})
